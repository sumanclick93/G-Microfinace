$(document).ready(function () {
    ...
    function initMobileCardView($table) {
        ...
        // Wait until DataTables has wrapped the table
        ...
        if ($wrapper.find('.mobile-view-toggle-container').length === 0) {
            var isCardDefault = $wrapper.hasClass('mobile-card-mode');
            var $toggleBtn = $('<button type="button" class="mobile-view-toggle-btn"></button>');
            if (isCardDefault) {
                $toggleBtn.html('<i class="ri-layout-grid-line"></i> <span>Card View (Active)</span>').css('background', '#0d6efd');
            } else {
                $toggleBtn.html('<i class="ri-table-line"></i> <span>Switch to Card View</span>').css('background', '#475569');
            }
    ...
    // Expose for page-level scripts (e.g. #collection_table)
    window.initMobileCardView = initMobileCardView;
    ...
    function retryMobileCardInit(attempt) {
        attempt = attempt || 0;
        ...
        // Retry a few times in case page DataTable init is delayed
        if (attempt < 5) {
            setTimeout(function () { retryMobileCardInit(attempt + 1); }, 300);
        }
    }
    setTimeout(function () { retryMobileCardInit(0); }, 300);
    ...
    function showDeleteConfirmation(customerId, customerName, role) {
        var title = 'Permanently Delete Customer?';
        ...
        var descText = 'Are you sure you want to <strong>permanently delete</strong> ' + customerName + '? This will erase all their historical records from the system.';
