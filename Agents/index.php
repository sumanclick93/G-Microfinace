<?php
// Include the database configuration (assuming it's one directory up)
include('config.php');

// If the agent is already logged in, redirect them to their dashboard
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        // Prepare a statement to query the AGENTS table
        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($password, $agent['password'])) {
                // Password is correct, create agent-specific session variables
...
<head>
    ...
    <title>Agent Login - G-Microfinance</title>
