...
// --- Fetch Dropdown Data (Agents & Customers) ---
$all_agents = [];
...
$all_customers = [];
$cust_res = $conn->query("SELECT id, full_name, customer_id_string FROM customers ORDER BY full_name ASC");
if ($cust_res) {
...
if ($filter_agent_id > 0) {
    $loan_where[] = "(p.collected_by_agent_id = " . intval($filter_agent_id) . " OR (p.collected_by_agent_id IS NULL AND c.agent_id = " . intval($filter_agent_id) . "))";
}
...
$loan_sql = "SELECT 
    ...
    COALESCE(p.collected_by_agent_id, c.agent_id) as agent_id,
    CONCAT(COALESCE(a.first_name, 'Direct'), ' ', COALESCE(a.last_name, '')) as agent_name
FROM payments p
JOIN loans l ON p.loan_id = l.id
JOIN customers c ON l.customer_id = c.id
LEFT JOIN agents a ON COALESCE(p.collected_by_agent_id, c.agent_id) = a.id
...
if ($filter_agent_id > 0) {
    $rd_where[] = "(p.collected_by_agent_id = " . intval($filter_agent_id) . " OR (p.collected_by_agent_id IS NULL AND c.agent_id = " . intval($filter_agent_id) . "))";
}
if (!empty($filter_start_date)) {
...
if ($filter_category === 'loan') {
    ...
    if ($filter_loan_type !== 'all') {
        // If a specific loan sub-type is selected, only query loans
        $final_sql = "$loan_sql ORDER BY payment_date DESC";
    } else {
        // Combine both Loan & RD payments with UNION
            ...
            <div class="page-body">
                <div class="container-fluid">
                    ...
                    <div class="row mb-3 align-items-center">
                        <div class="col-md-6">
                            <h3 class="fw-bold text-dark mb-1">All Payment Records</h3>
                            <p class="text-muted mb-0">Complete date-wise collection ledger (Loans & RD Deposits)</p>
                        </div>
                    </div>
                    ...
                    <div class="card shadow-sm border-0 mb-4">
                        <div class="card-header bg-white py-3 border-bottom">
                            <h6 class="mb-0 fw-bold text-dark"><i class="ri-filter-3-line me-2"></i>Filter Payment Records</h6>
                        </div>
                        <div class="card-body p-3">
                            <form method="GET" action="all-payments.php" class="row g-3">
                                ...
                                <div class="col-md-2 col-sm-6">
                                    ...
                                    <select name="agent_id" class="form-select form-select-sm">
                                        ...
                                        <?php foreach ($all_agents as $ag): ?>
                                            <option value="<?php echo $ag['id']; ?>" <?php if ($filter_agent_id == $ag['id']) echo 'selected'; ?>>
                                                <?php echo htmlspecialchars($ag['first_name'] . ' ' . $ag['last_name'] . ' (' . $ag['username'] . ')'); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                ...
                                <!-- Action Buttons -->
                                <div class="col-md-10 col-sm-12 d-flex align-items-end justify-content-end gap-2 ms-auto">
