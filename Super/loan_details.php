        ...
        if ($stmt_loan->execute()) {
            ...
            if ($result_loan->num_rows === 1) {
                ...
                if ($stmt_payments->execute()) {
                    ...
                    // --- 5. Calculate Remaining Balance & Monthly Interest Due ---
                    $remaining_balance = max(0, (float)$loan_details['total_repayable_amount'] - $total_paid);

                    $is_monthly_interest_api = (($loan_details['interest_calculation_type'] ?? '') === 'monthly_interest_only');
                    $accrued_months_api = 0;
                    $total_accrued_interest_api = 0.0;
                    $pending_interest_due_api = 0.0;
                    $pending_emis_count_api = 0;

                    if ($is_monthly_interest_api) {
                        $m_inst_api = (float)$loan_details['monthly_installment'];
                        if ($m_inst_api <= 0 && (float)$loan_details['loan_amount'] > 0 && (float)$loan_details['interest_rate'] > 0) {
                            $m_inst_api = round(((float)$loan_details['loan_amount'] * (float)$loan_details['interest_rate']) / 100, 2);
                        }
                        
                        $start_date_val_api = !empty($loan_details['loan_start_date']) ? $loan_details['loan_start_date'] : (!empty($loan_details['approval_date']) ? $loan_details['approval_date'] : $loan_details['application_date']);
                        $st_clean_api = strtolower(trim($loan_details['status']));

                        if (!empty($start_date_val_api) && !in_array($st_clean_api, ['rejected', 'pending'])) {
                            $start_dt_api = new DateTime($start_date_val_api);
                            $today_dt_api = new DateTime();
                            
                            if ($today_dt_api >= $start_dt_api) {
                                $ys_a = (int)$start_dt_api->format('Y');
                                $ms_a = (int)$start_dt_api->format('m');
                                $yt_a = (int)$today_dt_api->format('Y');
                                $mt_a = (int)$today_dt_api->format('m');
                                
                                $accrued_months_api = ($yt_a - $ys_a) * 12 + ($mt_a - $ms_a) + 1;
                                if ($accrued_months_api < 0) $accrued_months_api = 0;
                            }
                        }

                        if (in_array($st_clean_api, ['closed', 'paid'])) {
                            $pending_interest_due_api = 0.0;
                            $pending_emis_count_api = 0;
                            $total_accrued_interest_api = $total_paid;
                        } else {
                            $total_accrued_interest_api = $accrued_months_api * $m_inst_api;
                            $pending_interest_due_api = max(0, $total_accrued_interest_api - $total_paid);
                            $paid_months_calc_api = ($m_inst_api > 0) ? (int)floor($total_paid / $m_inst_api) : 0;
                            $pending_emis_count_api = max(0, $accrued_months_api - $paid_months_calc_api);
                        }
                    }
                    ...
                    $response['data'] = [
                        ...
                        // Open-ended monthly interest tracking
                        'accrued_months' => $accrued_months_api,
                        'total_accrued_interest' => round($total_accrued_interest_api, 2),
                        'pending_interest_due' => round($pending_interest_due_api, 2),
                        'pending_emis_count' => $pending_emis_count_api,
                        'pending_emi_description' => $is_monthly_interest_api ? ($pending_emis_count_api . ' Pending EMI(s) (₹' . number_format($pending_interest_due_api, 2) . ')') : null,
