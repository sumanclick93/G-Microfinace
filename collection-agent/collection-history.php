...
// Limit wide ranges to keep the pivot table usable
...
// Seed zero amounts for every date
...
// Loan collections from collection mirror table
$loan_sql = "
...
// RD collections from collection mirror table
$rd_sql = "
...
// Keep only customers who have at least one collected amount in range
...
// Keep only dates that have at least one collection (same layout, no empty day columns)
$dates = array_values(array_filter($dates, static function (string $d) use ($customers): bool {
    foreach ($customers as $c) {
        $day = $c['days'][$d] ?? ['loan' => 0, 'rd' => 0];
        if ((float)$day['loan'] > 0 || (float)$day['rd'] > 0) {
            return true;
        }
    }
    return false;
...
// If no collections at all, keep a single placeholder date so table header still renders
if ($dates === []) {
    $dates = [$filter_start_date];
}

// Dropdown list (all agent customers, unfiltered)
$all_customers = [];
...
// Totals for footer + WhatsApp summary
$grand_loan = 0.0;
$grand_rd   = 0.0;
$date_totals = [];
foreach ($dates as $d) {
                                ...
                                <div class="card-header border-0">
                                    <div class="card-header-title">
                                        ...
                                        <p class="text-muted mb-0" style="font-size: 13px;">
                                            Loan &amp; RD collections by customer and date for <?php echo htmlspecialchars($agent_name); ?>.
                                        </p>
                                    </div>
                                </div>
                                <div class="card-body">
                                    ...
                                    <form method="GET" action="collection-history.php" class="ch-filter-bar" id="historyFilterForm">
                                        <div class="row align-items-end g-2">
                                            ...
                                            <div class="col-md-5 col-sm-12 d-flex flex-wrap gap-2">
                                                ...
                                                <a href="collection-history.php" class="btn btn-outline-secondary btn-sm">
                                                    Today
                                                </a>
                                                <button type="button" class="btn btn-whatsapp btn-sm" id="btnShareWhatsApp">
                                                    <i class="ri-whatsapp-line"></i> Share via WhatsApp
                                                </button>
                                                <button type="button" class="btn btn-outline-danger btn-sm" id="btnDownloadPdf">
