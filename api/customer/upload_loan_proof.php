        ...
        if ($loan_id > 0 && $amount > 0 && isset($_FILES['proof'])) {
            ...
            if (in_array($file['type'], $allowed_types) && $file['error'] === 0) {
                ...
                if (move_uploaded_file($file['tmp_name'], $destination)) {
                    ...
                    $emi_count = $emi_count_stmt->get_result()->fetch_row()[0] + 1;
                    ...
                    if ($stmt->execute()) {
                        $stmt->close();
                        send_api_json_response(['status' => 'success', 'message' => 'Payment proof uploaded successfully. Pending Admin approval.'], 200, $conn);
                    } else {
                        $err = $stmt->error;
                        $stmt->close();
            ...
            send_api_json_response(['status' => 'error', 'message' => 'Missing loan_id, amount, or proof file.'], 400, $conn);
        }
    } else {
